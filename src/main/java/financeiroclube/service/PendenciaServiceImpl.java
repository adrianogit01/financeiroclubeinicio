...
public class PendenciaServiceImpl implements PendenciaService {
	...
	public Page<Pendencia> listarPagina(Pageable pagina) {
		...
		return pendenciaRepository.findAllByClubeOrderByDataEmissaoDescSocioAscNumeroAscParcelaAsc(clube, pagina);
	}
	...
	public void validar(Pendencia entidade, BindingResult validacao) {
		...
		if (entidade.getIdPendencia() == null) {
			if (entidade.getDataEmissao() != null && entidade.getSocio() != null
					&& pendenciaRepository.existsByNumeroAndParcelaAndDataEmissaoAndSocioAndClube(entidade.getNumero(),
							entidade.getParcela(), entidade.getDataEmissao(), entidade.getSocio(),
							usuarioService.lerLogado().getClube())) {
				validacao.rejectValue("socio", "Unique", new Object[] { 0, entidade.toString() }, null);
			}
		}
		
		else {
			if (entidade.getDataEmissao() != null && entidade.getSocio() != null
					&& pendenciaRepository.existsByNumeroAndParcelaAndDataEmissaoAndSocioAndClubeAndIdPendenciaNot(
							entidade.getNumero(), entidade.getParcela(), entidade.getDataEmissao(),
							entidade.getSocio(), usuarioService.lerLogado().getClube(),
	...
	public BigDecimal atraso() {
		...
		if (clube == null) {
			...
			resultado = pendenciaRepository.sumTotalByClubeAndDataVencimentoBeforeAndDataRecebimentoIsNull(clube,
					LocalDate.now());
			if (resultado == null) {
				resultado = BigDecimal.ZERO.setScale(2);
			}
	...
	public List<Pendencia> listarAtraso() {
		...
		if (clube != null && !clube.getPendencias().isEmpty()) {
			lista.addAll(pendenciaRepository
					.findAllByClubeAndDataVencimentoBeforeAndDataRecebimentoIsNullOrderBySocioAscDataVencimentoAsc(
							clube, LocalDate.now()));
		}
		return lista;
	}
