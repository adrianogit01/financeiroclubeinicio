...
public class RelatorioServiceImpl implements RelatorioService {
	...
	@Override
	public SortedMap<Socio, List<Pendencia>> atrasoAtualDetalhado() {
		SortedMap<Socio, List<Pendencia>> map = new TreeMap<>();
		List<Pendencia> atraso = pendenciaService.listarAtraso();
		for (Pendencia pendencia : atraso) {
			List<Pendencia> lista;
			if (map.containsKey(pendencia.getSocio())) {
				lista = map.get(pendencia.getSocio());
			} else {
				lista = new ArrayList<>();
			}
			lista.add(pendencia);
			map.put(pendencia.getSocio(), lista);
		}
		return map;
	}

	@Override
	public Map<Socio, BigDecimal> somaPendencias(Map<Socio, List<Pendencia>> map) {
		Map<Socio, BigDecimal> mapa = new HashMap<>();
		if (!map.isEmpty()) {
			for (Map.Entry<Socio, List<Pendencia>> entrada : map.entrySet()) {
